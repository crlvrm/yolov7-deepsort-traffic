...
from yolov7tiny.utils.utils import get_classes
cfg = get_config()
...
def get_labels(x):
    # yolov3
    # class_names, _ = get_classes('D:\\AboutPython\\CV\\unbox_yolov5_deepsort_counting-main\\yolov3\\model_data\\coco_classes.txt')
    class_names, _ = get_classes(r'D:\AboutPython\CV\unbox_yolov5_deepsort_counting-main\yolov7\model_data\coco_classes.txt')
    if x != '':
        predict = class_names[int(x)]
    else:
        predict = ''
    return predict

def draw_bboxes(image, bboxes,speed, line_thickness):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        t_size = cv2.getTextSize(get_labels(cls_id), 0, fontScale=line_thickness / 3, thickness=font_thickness)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(image, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(image, '{} ID-{} speed{}km/h'.format(get_labels(cls_id), pos_id,sp), (c1[0], c1[1] - 2), 0, line_thickness / 3,
                    [225, 255, 255], thickness=font_thickness, lineType=cv2.LINE_AA)

        list_pts.append([check_point_y - point_radius, check_point_x - point_radius])
        list_pts.append([check_point_y + point_radius, check_point_x - point_radius])
        list_pts.append([check_point_y + point_radius, check_point_x + point_radius])
        list_pts.append([check_point_y - point_radius, check_point_x + point_radius])
...
def update_tracker(target_detector, image):
        ...
        image = draw_bboxes(image, bboxes2draw)
